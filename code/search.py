...
pagemap = {}
pm = open(sys.argv[1]+'_pagemap').read()
pm = pm.split('\n')
for each in pm: 
	line = each.split(":")
	if len(line)>1:
		pagemap[int(line[0])] = line[1]
print "Loaded Pagemap"
		...
		if reqPath[0] == '/search':										# If the request is for a search process, read the query term.
			...
			for word in query:
				if not cache.has_key(word): 							# Check if word is in cache, making query faster
					search_results = search_query(word, index_file, 0, len(line_offset))
...
def search_query(query, index, i, j):
	...
	if mid < len(line_offset):
		index.seek(line_offset[mid])
		line = index.readline()
		currTerm = get_data(line)
